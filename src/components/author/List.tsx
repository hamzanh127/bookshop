        ...
        <tbody>
          {items.map((item) => (
            <tr key={item["@id"]}>
              ...
              <td>
                <Links
                  items={item["books"].map((ref: any) => ({
                    href: `/books/show/${encodeURIComponent(ref)}`,
                    name: ref,
                  }))}
                />
              </td>
              <td>
                <Link to={`/authors/show/${encodeURIComponent(item["@id"])}`}>
                  <span className="fa fa-search" aria-hidden="true" />
                  <span className="sr-only">Show</span>
                </Link>
              </td>
              <td>
                <Link to={`/authors/edit/${encodeURIComponent(item["@id"])}`}>
...
const List = () => {
  ...
  const id = (page && decodeURIComponent(page)) || "authors";
